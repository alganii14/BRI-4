...
class ImportMthFolder extends Command
{
    ...
    public function handle()
    {
        ...
        $this->info("📁 Ditemukan " . count($csvFiles) . " file CSV");
        $this->newLine();
        ...
        foreach ($csvFiles as $index => $file) {
            ...
            $this->info("[$fileNumber/$totalFiles] 📄 Memproses: {$fileName}");
            ...
            try {
                ...
                $successCount++;
                $this->info("✓ Berhasil import {$result['imported']} record dari {$fileName}");
                if ($result['skipped'] > 0) {
                    $this->warn("  ⚠ Dilewati: {$result['skipped']} records (duplikat/error)");
                }
            } catch (\Exception $e) {
                $failCount++;
                $this->error("✗ Gagal import {$fileName}");
